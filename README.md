...
- **原子文件写入** — 先写 `.tmp` 再复制，防止数据损坏
- **滚动日志** — 2MB 上限，3 个归档文件
- **诊断包导出** — ZIP 包含环境报告、日志、配置和历史记录
...
没有 FFmpeg 时，录屏会回退到内置 AVI（无音频，默认关闭）。
...
4. 选择：**复制**（剪贴板）、**保存**（PNG 文件）或 **钉图**（悬浮贴纸）

### 录屏流程

1. 按下 <kbd>F2</kbd> — 开始录屏，浮动状态窗出现
2. 正常工作 — 状态窗自动排除在录屏之外
3. 再次按下 <kbd>F2</kbd> — 停止录屏
...
| **通用** | 开机自启、关闭时最小化到托盘、历史保留天数（30/90/180/365） |
...
- **策略模式** — 录屏后端协调器，优先使用 FFmpeg，自动回退 AVI
- **双重热键注册** — Win32 `RegisterHotKey` 为主，低级键盘钩子为兜底
- **原子化持久化** — `JsonFileStore<T>` 先写 `.tmp` 再复制，防止数据损坏
...
测试套件包含 100+ 单元测试，覆盖模型、服务和工具类，使用 xUnit + Moq。

### 项目规范

- **根命名空间**：`SnipEasy.App`
- **代码风格**：StyleCop 分析器强制执行，`.editorconfig` 已配置
- **可空引用类型**：全局启用
- **隐式 using**：已启用
...
| 代码规范 | StyleCop.Analyzers |
| 安装包 | IExpress（PowerShell 构建） |
